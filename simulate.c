...
int user_parser P1(char *, buff)
{
	...
	*p = ' ';
	...
	if (illegal_sentence_action) {
	    ...
	    case 1:
		error("Illegal to call remove_action() [caller was /%s] from a verb(%s) returning zero.\n",illegal_sentence_ob->name,illegal_verb);
		break;
	    case 2:
		error("jason:Illegal to move or destruct an object (/%s) defining actions from a verb(%s) function which returns zero.\nuser (%s)",illegal_sentence_ob->name,buff,illegal_verb);
		fatal("quit for debug illegal move problem\n");
		break;
	    }
	}
    }
