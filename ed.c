...
static void report_status P1(int, status) {
	...
	ED_OUTPUT(ED_DEST, "致命错误\n");
	set_prompt(":");
	return;
#endif
    case CHANGED:
	ED_OUTPUT(ED_DEST, "当前文件已经改变。\n");
	break;
    case SET_FAIL:
	ED_OUTPUT(ED_DEST, "`set' 命令失败。\n");
	break;
    case SUB_FAIL:
	ED_OUTPUT(ED_DEST, "字串替换失败。\n");
	break;
    case MEM_FAIL:
	ED_OUTPUT(ED_DEST, "内存溢出：文件内容可能丢失！\n");
	break;
    case UNRECOG_COMMAND:
	ED_OUTPUT(ED_DEST, "不存在的命令。\n");
	break;
    case BAD_LINE_RANGE:
	ED_OUTPUT(ED_DEST, "错误的行范围。\n");
	break;
    case BAD_LINE_NUMBER:
	ED_OUTPUT(ED_DEST, "行数错误。\n");
	break;
    case SYNTAX_ERROR:
	ED_OUTPUT(ED_DEST, "命令语法错误\n");
	break;
    case RANGE_ILLEGAL:
	ED_OUTPUT(ED_DEST, "该命令无法使用范围。\n");
	break;
    case IS_RESTRICTED:
	ED_OUTPUT(ED_DEST, "该命令受限！\n");
	break;
    case LINE_OR_RANGE_ILL:
	ED_OUTPUT(ED_DEST, "这个命令无法使用行数或者范围。\n");
	break;
    case FILE_NAME_ERROR:
	ED_OUTPUT(ED_DEST, "文件操作命令失败。\n");
	break;
    case MARK_A_TO_Z:
	ED_OUTPUT(ED_DEST, "一个标记必须在 'a 到 'z的范围内。\n");
	break;
    case SUB_BAD_PATTERN:
	ED_OUTPUT(ED_DEST, "查找或替换的目标错误！\n");
	break;
    case SUB_BAD_REPLACEMENT:
	ED_OUTPUT(ED_DEST, "查找或替换的替换（匹配）失败。\n");
	break;
    case BAD_DESTINATION:
	ED_OUTPUT(ED_DEST, "复制或移动的目标错误。\n");
	break;
    case END_OF_FILE:
	ED_OUTPUT(ED_DEST, "文件末尾。\n");
	break;
    case SEARCH_FAILED:
	ED_OUTPUT(ED_DEST, "搜索失败。\n");
	break;
    default:
	ED_OUTPUT(ED_DEST, "命令失败。\n");
    }
}
...
static void print_help P1(int, arg)
{
	...
	break;
    case 'n':
	ED_OUTPUT(ED_DEST, "命令: n   用法: n\n");
	ED_OUTPUT(ED_DEST, "这个命令修改一个内部标志，使得一行被显示的时候可以根据这个标志\n");
	ED_OUTPUT(ED_DEST, "决定是否显示该行的行号。\n");
	break;
    case 'a':
	ED_OUTPUT(ED_DEST, "命令: a   用法: a\n");
	ED_OUTPUT(ED_DEST, "进入输入模式，并把输入的内容放到当前行后，在单独行输入'.'结束输入\n");
	break;
    case 'A':
	ED_OUTPUT(ED_DEST, "命令: A   用法: A\n\
Like the 'a' command, but uses inverse autoindent mode.\n");
	break;
    case 'i':
	ED_OUTPUT(ED_DEST, "命令: i   用法: i\n");
	ED_OUTPUT(ED_DEST, "进入输入模式，并把输入后的内容插入到当前行前面，单独行输入'.'结束输入\n");
	break;
    case 'c':
	ED_OUTPUT(ED_DEST, "命令: c   用法: c\n");
	ED_OUTPUT(ED_DEST, "进入输入模式，并把输入的内容取代当前行。\n");
	break;
    case 'd':
	ED_OUTPUT(ED_DEST, "命令: d   用法: d  或 [行号]d\n");
	ED_OUTPUT(ED_DEST, "无行号是删除当前行，负责删除指定行。\n");
	break;
#ifndef RESTRICTED_ED
    case 'e':
	ED_OUTPUT(ED_DEST, "命令: e  用法: e 文件名\n");
	ED_OUTPUT(ED_DEST, "装入新的文件进行编辑，如果当前文件已经改变，命令将会失败。\n");
	break;
    case 'E':
	ED_OUTPUT(ED_DEST, "命令: E  用法: E 文件名\n");
	ED_OUTPUT(ED_DEST, "装入新的文件进行编辑，如果当前文件已经改变，则放弃修改。\n");
	break;
    case 'f':
	ED_OUTPUT(ED_DEST, "命令: f  用法: f 或 f 文件名\n");
	ED_OUTPUT(ED_DEST, "无参数显示当前编辑的文件名，或带参数将当前文件名修改为参数指定的文件名。\n");
	break;
#endif				/* RESTRICTED_ED mode */
    case 'g':
	ED_OUTPUT(ED_DEST, "命令: g  用法: g/re/p\n");
	ED_OUTPUT(ED_DEST, "在所有行里查找're'指定的字符，并且对匹配的行进行p指定的操作\n");
	ED_OUTPUT(ED_DEST, "命令 'l' 就可以出现在'p'里\n");
	...
	break;
    case 'h':
	ED_OUTPUT(ED_DEST, "命令: h    用法:  h 或 hc (这里 c 是一个命令动词)\n");
	ED_OUTPUT(ED_DEST, "显示JackyBoy@CCTX翻译的命令帮助。\n");
	break;
    case 'j':
	ED_OUTPUT(ED_DEST, "命令: j    用法: j 或 [行号]j\n");
	ED_OUTPUT(ED_DEST, "合并行。缺省操作是合并当前行和续行。带行号操作我还不清楚，呵呵。\n");
	break;
    case 'k':
	ED_OUTPUT(ED_DEST, "命令: k   用法: kc  (这里c是一个字符)\n");
	ED_OUTPUT(ED_DEST, "给当前行一个标记c, 标记应该是一个小写字母。\n");
	break;
    case 'l':
	ED_OUTPUT(ED_DEST, "命令: l   用法: l  或  [行号]l\n");
	ED_OUTPUT(ED_DEST, "显示当前行或行号指定的行，行中的一些不可显字符以某个特殊标志显示出来。\n");
	break;
    case 'm':
	ED_OUTPUT(ED_DEST, "命令: m   用法: mADDRESS or [行号]mADDRESS\n");
	ED_OUTPUT(ED_DEST, "移动当前行(或指定的行号) 到ADDRESS指定的行后。Address为0表示是文件的开\n头，缺省值则是当前行。\n");
	break;
    case 'p':
	ED_OUTPUT(ED_DEST, "命令: p    用法: p  或  [行号]p\n");
	ED_OUTPUT(ED_DEST, "将当前或指定行输出到屏幕上，如果需要知道行号，请查看'n'命令的使用帮助。\n");
	break;
    case 'q':
	ED_OUTPUT(ED_DEST, "命令: q    用法: q\n");
	ED_OUTPUT(ED_DEST, "退出编辑器。如果有任何为保存的修改，则无法使用该命令退出。\n查看'w'命令获取保存文件的帮助。\n");
	break;
    case 'Q':
	ED_OUTPUT(ED_DEST, "命令: Q    用法: Q\n");
	ED_OUTPUT(ED_DEST, "强行退出编辑器，即使有未保存的修改也将其放弃掉。\n");
	break;
#ifndef RESTRICTED_ED
    case 'r':
	ED_OUTPUT(ED_DEST, "命令: r    用法: r 文件名\n");
	ED_OUTPUT(ED_DEST, "从指定文件中读取内容到缓存，两个文件合并。\n");
	break;
#endif				/* RESTRICTED_ED */
    case 't':
	ED_OUTPUT(ED_DEST, "命令: t   用法: tADDRESS 或 [行号]tADDRESS\n");
	ED_OUTPUT(ED_DEST, "复制当前行或指定行到ADDRESS指定的位置，缺省则复制到当前行后。\n");
	break;
    case 'v':
	ED_OUTPUT(ED_DEST, "命令: v   用法: v/re/p\n");
	ED_OUTPUT(ED_DEST, "在所有行中搜索表达式're', 并显示所有匹配的行。\n");
	ED_OUTPUT(ED_DEST, "'p'可以给出指定的命令操作。\n");
	ED_OUTPUT(ED_DEST, "相关命令参看 'g'.\n");
	break;
    case 'z':
	ED_OUTPUT(ED_DEST, "命令: z   用法: z  或  z-  或 z.\n");
	ED_OUTPUT(ED_DEST, "从当前行开始显示20行，如果命令是'z.'那么显示的20行将是以当前行为中心。\n命令'z-'显示当前行之前的20行。\n");
	break;
    case 'Z':
	ED_OUTPUT(ED_DEST, "命令: Z   用法: Z  or  Z-  or Z.\n");
	ED_OUTPUT(ED_DEST, "从当前行开始显示40行，如果命令是'Z.'那么显示的40行将是以当前行为中心。\n命令'Z-'显示当前行之前的40行。\n");
	break;
    case 'x':
	ED_OUTPUT(ED_DEST, "命令: x   用法: x\n");
	ED_OUTPUT(ED_DEST, "使用当前的文件名存储文件，并退出编辑器。\n");
	break;
    case 's':
...
typing ^D or ^K in the autoindent mode.\n");
	...
	break;
    case '/':
	ED_OUTPUT(ED_DEST, "命令: /   用法: /pattern\n");
	ED_OUTPUT(ED_DEST, "在缓存中寻找下一个字符串'pattern'.  这里的\
是一个表达式，\n所以象.*][()应该在前面加上\\.\n");
	break;
    case '?':
	ED_OUTPUT(ED_DEST, "命令: ?   用法: ?pattern\n");
	ED_OUTPUT(ED_DEST, "类似/命令，但是该命令是向前搜索，不是向后搜索！\n");
	break;
	/* someone should REALLY write help for s in the near future */
#ifndef RESTRICTED_ED
    case 'w':
    case 'W':
	ED_OUTPUT(ED_DEST, "还没有该命令的帮助，抱歉！\n");
	break;
#endif				/* RESTRICTED_ED */
    default:
	ED_OUTPUT(ED_DEST, "       Ed  (V 2.0)帮助文件\n");
	ED_OUTPUT(ED_DEST, "---------------------------------\n");
	ED_OUTPUT(ED_DEST, "     by Qixx [Update: 7/10/91]\n");
	ED_OUTPUT(ED_DEST, "     汉化 JackyBoy@CCTX 6/1/99 -- 儿童节快乐！\n");
	ED_OUTPUT(ED_DEST, "\n\n命令\n--------\n");
	ED_OUTPUT(ED_DEST, "/\t前向查找字符\n");
	ED_OUTPUT(ED_DEST, "?\t后向查找字符\n");
	/* ED_OUTPUT(ED_DEST, "^\tglobal search and print for pattern\n"); */
	ED_OUTPUT(ED_DEST, "=\t显示当前行是第几行\n");
	ED_OUTPUT(ED_DEST, "a\t在当前行后新增加一行\n");
	ED_OUTPUT(ED_DEST, "A\t类似'a'命令，但是将翻转自动缩进模式\n");
	ED_OUTPUT(ED_DEST, "c\t将当前行删除并重新输入\n");
	ED_OUTPUT(ED_DEST, "d\t删除指定范围的行\n");
#ifdef RESTRICTED_ED
	if (!P_RESTRICT) {
	    ED_OUTPUT(ED_DEST, "e\t退出当前档案, 开始编辑另一档(档案改变过未存盘无效)\n");
	    ED_OUTPUT(ED_DEST, "E\t类似'e'命令，但是文件被修改过也有效\n");
	    ED_OUTPUT(ED_DEST, "f\t显示或改变文件名\n");
	}
#endif				/* restricted ed */
	ED_OUTPUT(ED_DEST, "g\t查找匹配行并对其执行相应命令\n");
	ED_OUTPUT(ED_DEST, "h\t帮助文件(显示现在的这些信息)\n");
	ED_OUTPUT(ED_DEST, "i\t在当前行前插入一行\n");
	ED_OUTPUT(ED_DEST, "I\t排版整个代码 (Qixx version 1.0)\n");
	ED_OUTPUT(ED_DEST, "\n--按回车键继续--");
	P_MORE = 1;
	break;
    }
...
{
    P_MORE = 0;
    ED_OUTPUT(ED_DEST, "j\t合并行，默认是将后续行连接到当前行\n");
    ED_OUTPUT(ED_DEST, "k\t标记当前行- later referenced as 'a\n");
    ED_OUTPUT(ED_DEST, "l\t显示指定行(可显示控制字符)\n");
    ED_OUTPUT(ED_DEST, "m\t移动指定行(或几行)到指定位置\n");
    ED_OUTPUT(ED_DEST, "n\t行号显示切换开关\n");
    ED_OUTPUT(ED_DEST, "O\t同命令'i'\n");
    ED_OUTPUT(ED_DEST, "o\t同命令'a'\n");
    ED_OUTPUT(ED_DEST, "p\t输出指定范围行\n");
    ED_OUTPUT(ED_DEST, "q\t退出编辑器\n");
    ED_OUTPUT(ED_DEST, "Q\t退出编辑器，即使文件已经被修改且未存盘\n");
    ED_OUTPUT(ED_DEST, "r\t在文件尾或指定行后读进另一文件\n");
    ED_OUTPUT(ED_DEST, "s\t查找并替换(只对当前行第一个查找的字符串进行替换)\n");
    ED_OUTPUT(ED_DEST, "set\t查询，改变和保存编辑器的设定值\n");
    ED_OUTPUT(ED_DEST, "t\t复制指定行到指定位置\n");
    ED_OUTPUT(ED_DEST, "v\t搜索并对不匹配行执行指定命令\n");
    ED_OUTPUT(ED_DEST, "x\t保存文件并退出\n");
#ifdef RESTRICTED_ED
    if (!P_RESTRICT) {
	ED_OUTPUT(ED_DEST, "w\t将编辑的内容写到当前文件或指定的文件里\n");
	ED_OUTPUT(ED_DEST, "W\t类似'w'命令，但是是将编辑的内容附加到指定或当前文件后\n");
    }
#endif				/* restricted ed */
    ED_OUTPUT(ED_DEST, "z\t显示20行，可用参数 . + -\n");
    ED_OUTPUT(ED_DEST, "Z\t显示40行，可用参数 . + -\n");
    ED_OUTPUT(ED_DEST, "\n你可以输入'hc'得到更详细的帮助，这里c是你想得到帮助的命令\n");
}
#endif				/* ED */
